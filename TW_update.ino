...
void ParallelParking()
{
    ...
    // go forward till right wall disappea
    while (1)
    {
        SetSteering(0);
        SetSpeed(-0.5);
...
void T_Parking()
{
    ...
    while (1)
    {
        ...
        if (abs(GetDistance(R_TRIG, R_ECHO) - GetDistance(L_TRIG, L_ECHO)) < 90 && GetDistance(R_TRIG, R_ECHO) - GetDistance(L_TRIG, L_ECHO) > 10) //제대로 좌회전한 상황=>왼쪽 벾까지의 거리와 오른쪽 벽까지의 거리의 차가 90mm 미만일 때
        {
            SetSteering(0); //우선 정지
            SetSpeed(0);
            delay(1000);
    ...
    if (ir_sensing(IR_R) <= detect_ir && ir_sensing(IR_L) <= detect_ir) // 양쪽 차선이 검출된 경우=>정지선 또는 차선과 정지선을 모두 걸친 경우0
    {
        ...
        else if (GetDistance(FC_TRIG, FC_ECHO) < 300) //전방에 벽이 느껴질 때
        {
            while (1)
            {
                ...
                SetSpeed(0.1);
                delay(10);
                if (GetDistance(FC_TRIG, FC_ECHO) < 50) //전진해서 흰색선이 검출될 때까지
                {
                    SetSpeed(0);
...
void loop()
{
    ...
    /*  Serial.println(GetDistance(FC_TRIG, FC_ECHO));
      delay(100);
    */
    /* Serial.println(abs(GetDistance(R_TRIG, R_ECHO)-GetDistance(L_TRIG, L_ECHO)));
     delay(100);*/
}
